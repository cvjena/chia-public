...
class KerasIncrementalModel(ProbabilityOutputModel):
    def __init__(self, cls):
        ...
        self.feature_extractor.add(tf.keras.layers.Conv2D(filters=6, kernel_size=(3, 3), activation='relu', input_shape=(28, 28, 1)))
        self.feature_extractor.add(tf.keras.layers.AveragePooling2D())
        self.feature_extractor.add(tf.keras.layers.Conv2D(filters=16, kernel_size=(3, 3), activation='relu'))
        self.feature_extractor.add(tf.keras.layers.AveragePooling2D())
        ...
        self.feature_extractor.add(tf.keras.layers.Dense(units=10, activation='softmax'))
        self.feature_extractor.summary()

        # Add regularizer: see https://jricheimer.github.io/keras/2019/02/06/keras-hack-1/
        for layer in self.feature_extractor.layers:
            if isinstance(layer, tf.keras.layers.Conv2D) or isinstance(layer, tf.keras.layers.Dense):
                layer.add_loss(lambda layer=layer: tf.keras.regularizers.l2(5e-5)(layer.kernel))
    ...
    def build_image_batch(self, samples):
        ...
        return np.stack([sample.get_resource('input_img_np') for sample in samples], axis=0)
        ...
        with InstrumentationContext(self.__class__.__name__):
            ...
            for inner_step in range(inner_steps):
                ...
                with tf.GradientTape() as tape:
                    ...
                    reg_loss = sum(self.feature_extractor.losses + self.cls.regularization_losses())

                    total_loss = hc_loss + reg_loss

                total_trainable_variables = self.feature_extractor.trainable_variables + self.cls.trainable_variables()
