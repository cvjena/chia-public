...
class OneHotEmbeddingBasedKerasHierarchicalClassifier(EmbeddingBasedKerasHierarchicalClassifier):
    ...
    def update_embedding(self):
        with InstrumentationContext(self.__class__.__name__):
            ...
            self.fc_layer = tf.keras.layers.Dense(current_observed_concept_count, activation='softmax', kernel_regularizer=tf.keras.regularizers.l2(5e-5))

            report('observed_concepts', current_observed_concept_count)
            self.last_observed_concept_count = current_observed_concept_count

            self.dimension_to_uid = [concept.data['uid'] for concept in sorted(current_observed_concepts, key=lambda concept: concept.data['uid'])]
    ...
    def deembed_dist(self, embedded_labels):
        return [[(uid, embedded_label[dim]) for uid, dim in self.uid_to_dimension.items()] for embedded_label in embedded_labels]
